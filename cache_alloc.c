...
static int simple_task(void *arguments)
{
	...
	my_kmem_cache = kmem_cache_create("my_kmem_cache",sizeof(struct spage),0,SLAB_HWCACHE_ALIGN, NULL);//,NULL);
	
	if(!my_kmem_cache)
		return -ENOMEM;
	else
	{
		cachep = kmem_cache_alloc(my_kmem_cache,GFP_KERNEL);
	}
...
static void simple_exit (void) {
	...
	kmem_cache_free(my_kmem_cache,cachep);
